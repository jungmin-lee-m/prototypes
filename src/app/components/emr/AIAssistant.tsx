...
function PanelHeader({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex items-center justify-between px-4 h-[56px] border-b border-[var(--line-default)] flex-shrink-0 bg-white">
      <div className="flex items-center gap-2">
        {/* 햄버거 메뉴 — AI 패널 내 메뉴/사이드바 진입점. 캡처 화면 참고. */}
        <button
          title="메뉴"
          aria-label="메뉴"
          className="w-7 h-7 inline-flex items-center justify-center text-[var(--text-sub)] hover:text-[var(--text-main)] hover:bg-[var(--bg-subtle)] rounded transition-colors">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M2 4H14M2 8H14M2 12H14" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
          </svg>
        </button>
        <div className="w-[18px] h-[18px] bg-[var(--brand-primary-hover)] rounded-[4px] flex items-center justify-center flex-shrink-0 ml-0.5">
          <span className="text-white text-sm font-bold leading-none">✦</span>
        </div>
        <span className="text-lg font-medium text-[var(--text-main)]">AI 어시스턴트</span>
      </div>
      <div className="flex items-center gap-4">
        {/* 펼치기/접기(—) 버튼 제거됨 — 화면폭이 좁아질 경우 펼치기가 불가능하므로 닫기만 유지. */}
