    ...
    <div className="bg-white flex flex-col flex-shrink-0">
      ...
      <div className="px-3 pb-2">
        ...
        <div className="grid grid-cols-4 py-1">
          {["일자", "혈압", "맥박", "체온"].map((h) => (
            <span key={h} className="text-[10px] font-medium text-[var(--text-tertiary)] text-center">{h}</span>
          ))}
        </div>
        {vitals.map((v) => (
          <div key={v.date} className="grid grid-cols-4 py-1 border-t border-[var(--bg-subtle)]">
            <span className="text-[11px] text-[var(--text-main)] text-center">{v.date}</span>
            <span className={`text-[11px] font-medium text-center ${v.bpHigh ? "text-[var(--red-500)]" : "text-[var(--text-main)]"}`}>{v.bp}</span>
            <span className="text-[11px] font-medium text-[var(--text-main)] text-center">{v.hr}</span>
            <span className="text-[11px] font-medium text-[var(--text-main)] text-center">{v.temp}</span>
          </div>
        ))}
      </div>
    </div>
